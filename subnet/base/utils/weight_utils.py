...
def process_weights_for_netuid(
    ...
    if non_zero_weights.size == 0 or metagraph.n < min_allowed_weights:
        # No meaningful weights to set (or the metagraph is too small). Return a uniform
        # distribution rather than concentrating all weight on an arbitrary UID.
        bittensor.logging.warning(
            "No non-zero weights available (or metagraph too small); returning uniform weights."
        )
